...
static void handle_syscall(trapframe *tf) {
  ...
  long rtn = do_syscall(tf->regs.a0,tf->regs.a1,tf->regs.a2,tf->regs.a3,
  ...
  switch (mcause) {
    case CAUSE_STORE_PAGE_FAULT:
      ...
      if(stval < USER_STACK_TOP && stval > USER_STACK_TOP - (20 * STACK_SIZE)){
        void *np = alloc_page();
        user_vm_map((pagetable_t)current->pagetable, ROUNDDOWN(stval, PGSIZE), PGSIZE, (uint64)np, prot_to_type(PROT_WRITE | PROT_READ, 1));
        ++current->allocPage;
      }

      if(stval < USER_STACK_TOP - (current->allocPage * STACK_SIZE)){
          panic("this address is not available!");
      }
//      else if(stval < USER_STACK_TOP - (20 * STACK_SIZE)){
//          panic("this address is not available!");
//      }
